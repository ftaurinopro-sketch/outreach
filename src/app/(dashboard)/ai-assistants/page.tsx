  ...
  return (
    <div className="max-w-4xl px-8 py-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900">AI Assistants</h1>
          <p className="mt-1 text-sm text-neutral-500">
            Crea un agent per ogni use case. Ognuno ha il proprio tono, obiettivo e contesto aziendale.
          </p>
        </div>
        <Link
      ...
      {agents.length === 0 ? (
        <div className="mt-8 rounded-lg border border-dashed border-neutral-300 bg-white px-5 py-10 text-center text-sm text-neutral-500">
          Nessun agent ancora. Creane uno per iniziare.
        </div>
      ) : (
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {agents.map((agent) => (
            <li key={agent.id}>
              ...
              >
                <div className="font-medium text-neutral-900">{agent.name || "(senza nome)"}</div>
